# ROADMAP archive — 2026-09-06 (v7 cleanup)

Verbatim record of the completed work up to v6.1, moved out of ROADMAP.md at the v7
decision (proposal §12) to keep the active queue context-clean. Nothing here was
edited: Phases 0–2 + the v6.1 follow-ups (T0.1–T2.26) and the complete Notes log
through 2026-09-06. Gate/forensic history (phase-2 gate, fabrication episodes, review
arbitrations) lives in the Notes log below and in docs/verify/. New Notes entries go
to ROADMAP.md's fresh "Notes log (v7 era)" section.
...
- [ ] **T2.17 `DOC` `MANUAL-GATE` — `docs/verify/phase2.md` (v6)** *(gate doc written 2026-09-04, commit c772332; awaiting the human on the Pi — step 1 is the $GAPI STOP gate)* — full rewrite;
  ...
  *(⚠️ v7 note 2026-09-06 — proposal §12: superseded in part. The knowledge_sync/
  search steps of this gate never ran and never will — that machinery is deleted in
  T2.28. The doc stays as historical record; its still-live checks ($GAPI
  credential, Telegram door) fold into T2.35.)*
...
- [ ] **T2.21 `DOC` — phase-2 gate refresh for v6.1** — update `docs/verify/phase2.md`
  ...
  *(⚠️ SUPERSEDED 2026-09-06 — v7 / proposal §12: the v6.1 sync machinery this
  refresh targets is deleted in T2.28; the fresh gate run will never happen. Folded
  into T2.35. Box stays unchecked; the DOC work remains as history.)*
...
- [ ] **T2.24 `OPTIONAL` — Drive Changes API cursor (proposal §3)** — replace the
  ...
  *(⚠️ SUPERSEDED 2026-09-06 — v7 / proposal §12: no watermark exists anymore — the
  Drive→cache sync is deleted in T2.28. Deletion detection is moot; the KB record is
  git-versioned locally and snapshotted daily.)*
